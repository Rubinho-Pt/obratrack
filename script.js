...
function saveTasks() { localStorage.setItem("obraTrackTasks", JSON.stringify(tasks)); }
...
function applyLanguage(language) {
  ...
  localStorage.setItem("obraTrackLanguage", currentLanguage);
...
function toggleTask(id) {
  ...
  saveTasks(); updateStats(); renderTasks(); showToast(completed?text().taskCompleted:text().taskReopened);
}
function deleteTask(id) { tasks=tasks.filter(task=>task.id!==id); saveTasks(); updateStats(); renderTasks(); showToast(text().taskDeleted); }
